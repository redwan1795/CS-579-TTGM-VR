...
public class CubeSummonerScript : MonoBehaviour
{
    ...
    private bool isEnabledByButton;

    private void Start()
    {
        isEnabledByButton = true;
    }
    ...
    void Update()
    {
        ...
        */
        ...
        if (OVRInput.GetDown(OVRInput.Button.One) && isEnabledByButton == true && (editorMode < 1 || editorMode > 10))
        //if (Input.GetKeyDown(KeyCode.H) && (editorMode<1 || editorMode > 10))
        {
            isEnabledByButton = false;
            upDateMode(0);

        }
        // returns true if the “A” button was released this frame.
        if (OVRInput.GetDown(OVRInput.Button.One) && isEnabledByButton == false)
